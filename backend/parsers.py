...
#
# Parsers accept an optional `ctx` dict carrying provenance computed by the
# Analyzer, so findings chain off the artifact that produced them instead of
# anchoring everything to the machine node (a useless star topology). Keys:
#   source_id        — the node a finding should hang off (endpoint/file/service)
#   foothold_user_id — the user node we have a shell as (for privesc chaining)
#   foothold_cred_id — the credential node that yielded the foothold
# All keys are optional; absent provenance falls back to the machine node.


def _src(ctx: dict | None, target: str) -> str:
    """The node a finding should anchor to — its provenance, or the machine."""
    return (ctx or {}).get('source_id') or target

def parse_nmap(output: str, target: str, ctx: dict | None = None) -> dict:
    try:
        nodes, edges = [], []
...
def parse_gobuster(output: str, target: str, ctx: dict | None = None) -> dict:
    try:
        nodes, edges = [], []
        src = _src(ctx, target)
        ...
        for m in re.finditer(r'(/\S+)\s+\(Status:\s*(\d+)\)', output):
            ...
            if not any(n['id'] == node_id for n in nodes):
                ...
                edges.append({'source': src, 'target': node_id, 'label': 'path'})
        # Cap at 15 nodes
        if len(nodes) > 15:
            nodes = nodes[:15]
            edges = edges[:15]
...
def parse_ffuf(output: str, target: str, ctx: dict | None = None) -> dict:
    try:
        nodes, edges = [], []
        src = _src(ctx, target)
...
def parse_nuclei(output: str, target: str, ctx: dict | None = None) -> dict:
    try:
        nodes, edges = [], []
        src = _src(ctx, target)
        for m in re.finditer(r'\[([^\]]+)\]\s+\[([^\]]+)\]\s+\[([^\]]+)\]\s+(\S+)', output):
            ...
            edges.append({'source': src, 'target': node_id, 'label': severity})
        return {'nodes': nodes, 'edges': edges}
    except Exception:
        return {'nodes': [], 'edges': []}


def parse_searchsploit(output: str, target: str, ctx: dict | None = None) -> dict:
    try:
        nodes, edges = [], []
        src = _src(ctx, target)
        for m in re.finditer(r'(\S.*\S)\s+\|\s+(exploits/\S+|shellcodes/\S+)', output):
            ...
            edges.append({'source': src, 'target': node_id, 'label': 'exploit'})
        return {'nodes': nodes, 'edges': edges}
    except Exception:
        return {'nodes': [], 'edges': []}


def parse_pcap_analysis(output: str, target: str, ctx: dict | None = None) -> dict:
    try:
        nodes, edges = [], []
        # Anchor recovered creds to the artifact they came from (the pcap/file
        # node), not the machine — this is the whole point of the graph.
        src = _src(ctx, target)
        ...
        for user in ftp_users:
            ...
            edges.append({'source': src, 'target': node_id, 'label': 'cred found'})
        ...
        for m in re.finditer(r'Authorization:\s*Basic\s+(\S+)', output):
            ...
            edges.append({'source': src, 'target': 'vuln-basic-auth', 'label': 'credential'})
...
def parse_sqlmap(output: str, target: str, ctx: dict | None = None) -> dict:
    ...
    try:
        ...
        for m in re.finditer(r"Parameter:\s*(\S+)\s.*is\s+(.*vulnerable)", output, re.IGNORECASE):
            ...
            first_sqli = first_sqli or node_id
            nodes.append({'id': node_id, 'label': f'SQLi: {param}', 'type': 'vulnerability'})
            edges.append({'source': src, 'target': node_id, 'label': 'sqli'})
        # Detect databases — chain them off the injection point if we found one.
        db_src = first_sqli or src
        for m in re.finditer(r'\[\*\]\s+(\w+)', output):
            ...
            if db not in ('information_schema', 'performance_schema', 'mysql', 'sys', 'starting', 'testing', 'shutting'):
                ...
                edges.append({'source': db_src, 'target': node_id, 'label': 'database'})
        return {'nodes': nodes, 'edges': edges}
    except Exception:
        return {'nodes': [], 'edges': []}


def parse_hydra(output: str, target: str, ctx: dict | None = None) -> dict:
    ...
    try:
        ...
        for m in re.finditer(r'\[(\d+)\]\[(\w+)\]\s+host:\s+\S+\s+login:\s+(\S+)\s+password:\s+(\S+)', output):
            ...
            # Chain the cracked cred off the service it was brute-forced against.
            edges.append({'source': f'{target}:{port}', 'target': node_id, 'label': f'{service} brute'})
        return {'nodes': nodes, 'edges': edges}
    except Exception:
        return {'nodes': [], 'edges': []}


def parse_wpscan(output: str, target: str, ctx: dict | None = None) -> dict:
    ...
    try:
        ...
        for m in re.finditer(r'\|\s+(\w+)\s+\|.*Author', output):
            ...
            edges.append({'source': src, 'target': node_id, 'label': 'wordpress'})
        ...
        for m in re.finditer(r'Title:\s*(.+)', output):
            ...
            nodes.append({'id': node_id, 'label': title, 'type': 'vulnerability'})
            edges.append({'source': src, 'target': node_id, 'label': 'wp-vuln'})
        return {'nodes': nodes, 'edges': edges}
    except Exception:
        return {'nodes': [], 'edges': []}
